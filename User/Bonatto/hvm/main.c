          ...
          //GENERATED_REWRITE_RULES_STEP_1_START//
          {
            case _SUM_: {
              ...
              if ((get_tag(ask_arg(mem, term, 0)) == CTR && get_ext(ask_arg(mem, term, 0)) == 32u)) {
                ...
                u64 ret_0;
                if (get_tag(Cal(1, 29, cal_2)) == NUM && get_tag(Cal(1, 29, cal_3)) == NUM) {
                  ret_0 = Num(get_num(Cal(1, 29, cal_2)) + get_num(Cal(1, 29, cal_3)));
                  inc_cost(mem);
                } else {
                  u64 op2_1 = alloc(mem, 2);
                  link(mem, op2_1 + 0, Cal(1, 29, cal_2));
                  link(mem, op2_1 + 1, Cal(1, 29, cal_3));
                  ret_0 = Op2(ADD, op2_1);
                }
                u64 done = ret_0;
                link(mem, host, done);
            ...
            case _MAIN_: {
              if (1) {
                ...
                link(mem, cal_1 + 0, Cal(1, 30, cal_0));
                u64 done = Cal(1, 29, cal_1);
                link(mem, host, done);
                clear(mem, get_loc(term, 0), 1);
                init = 1;
                continue;
            ...
            case _GEN_: {
              ...
              if ((get_tag(ask_arg(mem, term, 0)) == CTR || get_tag(ask_arg(mem, term, 0)) == NUM)) {
                ...
                u64 ret_5;
                if (get_tag(dp0_1) == NUM && get_tag(Num(1ull)) == NUM) {
                  ret_5 = Num(get_num(dp0_1) - get_num(Num(1ull)));
                  inc_cost(mem);
                } else {
                  u64 op2_6 = alloc(mem, 2);
                  link(mem, op2_6 + 0, dp0_1);
                  link(mem, op2_6 + 1, Num(1ull));
                  ret_5 = Op2(SUB, op2_6);
                }
                u64 cal_7 = alloc(mem, 1);
                link(mem, cal_7 + 0, ret_5);
                u64 ret_8;
                if (get_tag(dp1_2) == NUM && get_tag(Num(1ull)) == NUM) {
                  ret_8 = Num(get_num(dp1_2) - get_num(Num(1ull)));
                  inc_cost(mem);
                } else {
                  u64 op2_9 = alloc(mem, 2);
                  link(mem, op2_9 + 0, dp1_2);
                  link(mem, op2_9 + 1, Num(1ull));
                  ret_8 = Op2(SUB, op2_9);
                }
                u64 cal_10 = alloc(mem, 1);
                link(mem, cal_10 + 0, ret_8);
                u64 ctr_11 = alloc(mem, 2);
                link(mem, ctr_11 + 0, Cal(1, 30, cal_7));
                link(mem, ctr_11 + 1, Cal(1, 30, cal_10));
                u64 done = Ctr(2, 32, ctr_11);
                link(mem, host, done);
                clear(mem, get_loc(term, 0), 1);
                init = 1;
                continue;
